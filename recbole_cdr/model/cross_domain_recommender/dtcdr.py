...
class DTCDR(CrossDomainRecommender):
    ...
    def predict_source(self, interaction):
        ...
        num_user = len(user) // self.source_num_items
        item = torch.arange(self.target_num_items - self.overlapped_num_items, self.total_num_items, device=self.device).repeat(num_user)
        if self.base_model == 'NeuMF':
            output = self.neumf_forward(user, item, 'source')
            return output
        else:
            output = self.dmf_forward(user, item, 'source')
            return output
    ...
    #     user = interaction[self.SOURCE_USER_ID]
    #     item = torch.arange(self.total_num_items, device=self.device)

    #     user_eval = user.repeat_interleave(len(item))
    #     item_eval = item.repeat(len(user))

    #     if self.base_model == 'NeuMF':
    #         output = self.neumf_forward(user_eval, item_eval, 'source')
    #         return output.view(-1)
    #     else:
    #         output = self.dmf_forward(user_eval, item_eval, 'source')
    #         return output.view(-1)
    ...
    #     if self.base_model == 'NeuMF':
    #         output = self.neumf_forward(user_eval, item_eval, 'target')
    #         return output.view(-1)
    #     else:
    #         output = self.dmf_forward(user_eval, item_eval, 'target')
    #         return output.view(-1)
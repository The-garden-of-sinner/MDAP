...
class BiTGCF(CrossDomainRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.domain_lambda_source = config['lambda']  # float32 type: the weight of source embedding in transfer function
        self.domain_lambda_target = config['lambda']  # float32 type: the weight of target embedding in transfer function
    ...
    def transfer_layer(self, source_all_embeddings, target_all_embeddings):
        ...
        source_alltransfer_embeddings = torch.cat([source_transfer_user_embeddings, source_item_embeddings], dim=0)
        target_alltransfer_embeddings = torch.cat([target_transfer_user_embeddings, target_item_embeddings], dim=0)

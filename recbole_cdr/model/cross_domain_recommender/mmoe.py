...
class MMOE(CrossDomainRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        #     MLPLayers([self.bottom_mlp_dims[-1]] + self.tower_mlp_dims, self.drop_rate) for i in range(self.n_experts)
        # ])
        ...
        # self.gate = torch.nn.ModuleList([torch.nn.Sequential(
        #     torch.nn.Linear(self.latent_dim * 2, self.n_experts),
    ...
    def source_forward(self, user, item, input=None):
        if input == None:
            ...
            source_input = input
        # gate = self.gate[0](source_input).unsqueeze(1)
        gate = self.gate_source(source_input).unsqueeze(1)
        fea = torch.cat([self.expert[i](source_input).unsqueeze(1) for i in range(self.n_experts)], dim = 1)
        task_fea = torch.bmm(gate, fea).squeeze(1)
        # source_out = torch.sigmoid(self.tower[0](task_fea).squeeze(1))
        source_out = self.tower_source(task_fea)
        source_out = self.predict_layer_source(source_out).squeeze(1)
        return source_out
    ...
    def target_forward(self, user, item, input=None):
        if input == None:
            ...
            target_input = input
        # gate = self.gate[1](target_input).unsqueeze(1)
        gate = self.gate_target(target_input).unsqueeze(1)
        fea = torch.cat([self.expert[i](target_input).unsqueeze(1) for i in range(self.n_experts)], dim = 1)
        task_fea = torch.bmm(gate, fea).squeeze(1)
        # target_out = torch.sigmoid(self.tower[1](task_fea).squeeze(1))
        target_out = self.tower_target(task_fea)
        target_out = self.predict_layer_target(target_out).squeeze(1)
        return target_out

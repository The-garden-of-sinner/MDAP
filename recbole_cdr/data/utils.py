...
from recbole.data.utils import load_split_dataloaders, save_split_dataloaders, create_samplers, _get_AE_dataloader
...
def data_preparation(config, dataset):
    ...
    if dataloaders is not None:
        ...
        source_train_sampler, source_valid_sampler, source_test_sampler = \
            create_source_samplers(config, dataset, built_datasets[:3])
        ...
        source_test_data = get_dataloader(config, 'evaluation', 'source')(config, dataset, source_test_dataset, source_test_sampler, shuffle=False)
        target_test_data = get_dataloader(config, 'evaluation', 'target')(config, target_test_dataset, target_test_sampler, shuffle=False)
        test_data = (source_test_data, target_test_data)
...
def get_dataloader(config, phase, domain='target'):
    ...
    model_type = config['MODEL_TYPE']
    if phase == 'train':
        if config['use_user_loader']:
            return CrossDomainUserDataloader
...
def create_source_samplers(config, dataset, built_datasets):
    ...
    Returns:
        tuple:
            ...
            - test_sampler (AbstractSampler): The sampler for test.
    """
